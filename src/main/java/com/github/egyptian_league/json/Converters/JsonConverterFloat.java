...
public class JsonConverterFloat extends JsonConverter<Float> {
    ...
    @Override
    public void serialize(Queue<JsonToken> tokens, Float value, JsonSerializerOptions options) {
        tokens.add(new JsonToken(value.toString(), JsonTokenType.NUMBER));
    }

    @Override
    public Float deserialize(JsonElement element, Type typeToConvert, JsonSerializerOptions options) {
